...
<section class="pagina-pedidos-fondo">
    <div class="panel">
        ...
        <form method="get" class="consulta-inline">
            <label for="IDUsuario">Proporcione el IDUsuario del cliente a consultar:</label>
            <input type="text" id="IDUsuario" name="IDUsuario" value="<?php echo htmlspecialchars($id, ENT_QUOTES, 'UTF-8'); ?>">
            <button type="submit">Consultar</button>
        </form>
        <?php if ($mensaje): ?>
            <div class="mensaje error"><?php echo htmlspecialchars($mensaje, ENT_QUOTES, 'UTF-8'); ?>
            </div><?php endif; ?>
        <?php if ($cliente): ?>
            <p>
                <strong>Cliente:</strong> <?php echo htmlspecialchars($cliente['IDUsuario'], ENT_QUOTES, 'UTF-8'); ?>
                <strong>Nombre:</strong> <?php echo htmlspecialchars($cliente['Nombre'] . ' ' . $cliente['ApellidoPaterno'] . ' ' . $cliente['ApellidoMaterno'], ENT_QUOTES, 'UTF-8'); ?>
            </p>
            ...
            <?php if ($pedidos): ?>
                ...
                <div class="mensaje info">El cliente no tiene pedidos registrados.</div>
            <?php endif; ?>
        <?php endif; ?>
    </div>
